...
class RecoveryDecisionEngine:
    def decide(self, request: ListingSyncRequest, comparison: Dict[str, Any], remote_state: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if "offer_missing_remote" in drifts:
            ...
            reason_codes.append("remote_offer_not_found")
            review_required = True
            recoverable = False

        elif "missing_ebay_listing_row" in drifts or "missing_listing_id_in_db" in drifts:
            ...
            if request.allow_recover_inventory:
                action = "reconcile_remote_from_db" # DB is source of truth for price/qty
                reason_codes.append("syncing_remote_to_db_baseline")
            else:
                action = "reconcile_db_from_remote" # Treat remote as truth
